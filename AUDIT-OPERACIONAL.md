...
## Estado visível após a correção do carregamento

A captura de runtime da rota `/` após o restart do servidor confirmou que a aplicação chega ao dashboard do Studio OS. A tela mostra `0 projetos`, `0 revisões`, `0 clientes` e `R$ 0,00` como dados persistidos vazios, sem números demonstrativos. O monitor de sessão mostra `SEM ENTRADA` e `AGUARDANDO`, com `FONTE DE ÁUDIO NÃO SELECIONADA`, portanto não promete uma medição de áudio inexistente. A tela inicial deixou de usar percentual de espera como informação de progresso de autenticação: o `LoadingScreen` agora usa indicador indeterminado e o `useAuthGate` sai após 3 segundos para um dashboard com aviso explícito de `Modo local ativo` quando o núcleo online não responde.
