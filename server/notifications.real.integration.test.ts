...
import { clients, deliveries, instrumentals, licenseOffers, projectActivities, projects, sales, studioNotifications, users } from "../drizzle/schema";
...
describe.skipIf(!realDbEnabled)("studio notifications com banco real", () => {
  it("persiste eventos de aprovação, comentário e finanças e lê por userId", async () => {
    ...
    let userId = 0; let clientId = 0; let projectId = 0; let deliveryId = 0; let instrumentalId = 0; let offerId = 0; let saleId = 0;
    try {
      ...
      const sale = await caller.studio.createSale({ clientId, licenseOfferId: offerId });
      saleId = Number((await db.select().from(sales).where(eq(sales.clientId, clientId)))[0]?.id || 0);
      expect(sale.amountCents).toBe(1000);
